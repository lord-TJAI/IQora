import React from 'react';
import { Card } from '@/components/ui/Card';
import { Button } from '@/components/ui/Button';
import { Input } from '@/components/ui/Input';
import { useAuthStore } from '@/stores/authStore';
import { useUIStore } from '@/stores/uiStore';

export const TeacherSettings: React.FC = () => {
  const { teacherData } = useAuthStore();
  const { addToast } = useUIStore();

  const handleSave = () => {
    addToast('Teacher preferences updated!', 'success');
  };

  return (
    <div className="space-y-6 max-w-2xl mx-auto animate-in fade-in duration-200">
      <div>
        <h1 className="text-2xl sm:text-3xl font-black text-brand-text-primary tracking-tight">
          Teacher Settings
        </h1>
        <p className="text-sm text-brand-text-secondary mt-1">
          Manage notification triggers, AI grading thresholds, and class preferences
        </p>
      </div>

      <Card className="p-6 border border-brand-border space-y-4 bg-white">
        <Input
          label="Display Name"
          defaultValue={teacherData?.title || 'Ms. Ananya Sharma'}
        />
        <Input
          label="Institutional Email"
          defaultValue={teacherData?.email || 'ananya.sharma@iqora.edu'}
          disabled
        />

        <div className="space-y-3 pt-3 border-t border-brand-border">
          <span className="text-xs font-bold text-brand-text-primary uppercase tracking-wider block">
            AI Assistant Configuration
          </span>
          <label className="flex items-center gap-3 text-xs font-semibold text-brand-text-primary cursor-pointer">
            <input type="checkbox" defaultChecked className="rounded text-brand-primary" />
            <span>Enable automated AI rubric suggestions for subjective submissions</span>
          </label>
          <label className="flex items-center gap-3 text-xs font-semibold text-brand-text-primary cursor-pointer">
            <input type="checkbox" defaultChecked className="rounded text-brand-primary" />
            <span>Notify me immediately when class concept gaps exceed 15 students</span>
          </label>
        </div>

        <div className="pt-4 border-t border-brand-border flex justify-end">
          <Button variant="primary" onClick={handleSave}>
            Save Preferences
          </Button>
        </div>
      </Card>
    </div>
  );
};
